...
spec = do
  describe "deleteFindMaxPH" $ do
    it "deleteFindMaxPH [1, 2] == Just (2, [1])" $ do
      deleteFindMaxPH @Int [1, 2] `shouldBe` Just (2, [1])
    it "deleteFindMaxPH [2, 1] == Just (2, [1])" $ do
      deleteFindMaxPH @Int [2, 1] `shouldBe` Just (2, [1])
    it "deleteFindMaxPH [1, 1] == Just (1, [1])" $ do
      deleteFindMaxPH @Int [1, 1] `shouldBe` Just (1, [1])
    it "deleteFindMaxPH [1] == Just (1, MaxEmpty)" $ do
      deleteFindMaxPH @Int [1] `shouldBe` Just (1, MaxEmpty)
    it "deleteFindMaxPH [] == Nothing" $ do
      deleteFindMaxPH @Int ([] :: MaxHeap Int) `shouldBe` Nothing
