{-# LANGUAGE CPP, DataKinds, KindSignatures, OverloadedLists      #-}
...
spec = do
    ...
    describe "fast zeta/moebius transform" $ do
        describe "SetOrd" $ withNatProxy @5 $ \natProxy -> do
        ...
        describe "DivOrd" $ withPrimes MAX_PRIME $ \primes -> do
...
posetSpec
    :: forall a.(Poset a, Eq a, Show a, Arbitrary a)
    => Proxy a -> Spec
posetSpec _ = do
...
latticeSpec
    :: forall a.(Lattice a, Eq a, Show a, Arbitrary a)
    => Proxy a -> Spec
latticeSpec _ = do
